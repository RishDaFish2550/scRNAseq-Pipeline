...
target_n  <- 30091                             
out_png   <- file.path("FIGURES_UMAP", sprintf("%s_S5_umap.png", sample_id))
...
cells_S5 <- colnames(raw)[ord][seq_len(min(target_n, ncol(raw)))]

cat(sprintf("[S5] Using top %d barcodes by total counts\n", length(cells_S5)))

obj <- CreateSeuratObject(
  counts = raw[, cells_S5, drop = FALSE],
  project = paste0(sample_id, "_S5"),
  min.cells = 3,
  min.features = 200
)
